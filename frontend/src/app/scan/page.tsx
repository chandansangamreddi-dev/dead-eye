...
type ScanMode = "screenshot" | "link" | "qr";
...
function saveActivity(
  ...
  try {
    const existing = localStorage.getItem("dead-eye-activity");
    const activities: ActivityItem[] = existing
      ? JSON.parse(existing)
      : [];
    ...
    const item: ActivityItem = {
      ...
      threat:
        result.analysis.risk_level === "SAFE"
    ...
    const updated = [item, ...activities].slice(0, 10);
...
export default function ScanPage() {
  ...
  const [guardianMode, setGuardianMode] =
    useState(false);
  ...
  const [scanMode, setScanMode] =
    useState<ScanMode>("screenshot");

  const [qrUrl, setQrUrl] =
    useState<string | null>(null);

  const [isAnalyzingQr, setIsAnalyzingQr] =
    useState(false);

  const [linkInput, setLinkInput] =
    useState("");

  const [isAnalyzingLink, setIsAnalyzingLink] =
    useState(false);

  /*
   * Read the mode from:
   *
   * /scan?mode=screenshot
   * /scan?mode=link
   * /scan?mode=qr
   *
   * We intentionally use window.location instead of
   * useSearchParams so this page can remain statically
  ...
  useEffect(() => {
    ...
    const savedGuardian =
      localStorage.getItem("dead-eye-guardian");
  ...
  const analyzeLink = async () => {
    ...
    try {
      const response = await fetch(
        "/api/analyze-text",
        {
          ...
          body: JSON.stringify({
            ...
            text: `A URL was submitted to DEAD EYE for security analysis.\nURL: ${normalizedUrl}`,
            url: normalizedUrl,
          }),
        },
      );
      ...
      if (!response.ok) {
        throw new Error(
          data?.detail ||
            `Link analysis failed with status ${response.status}`,
        );
      }

      setResult(data);
      saveActivity(data, "link");
    } catch (err) {
      console.error(err);
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Something went wrong while analyzing the link.",
      );
    } finally {
      setIsAnalyzingLink(false);
    }
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#060809] text-white">
      ...
      <div className="relative mx-auto min-h-screen max-w-md px-5 pb-10 pt-7">
        ...
        {/* Mode switch */}
        <div className="relative z-50 mt-7 grid grid-cols-3 rounded-2xl border border-white/[0.08] bg-white/[0.03] p-1">
          <button
            type="button"
            onClick={() => switchMode("screenshot")}
            className={`relative z-50 rounded-xl px-2 py-3 text-[11px] font-semibold transition ${
              scanMode === "screenshot"
          ...
          <section className="mt-8">
            ...
            <p className="mt-3 max-w-[310px] text-sm leading-5 text-white/40">
              {scanMode === "qr"
                ...
                : scanMode === "link"
                  ? "DEAD EYE inspects suspicious URLs for phishing and social engineering signals before you interact with them."
        ...
        {/* Link Scanner */}
        {scanMode === "link" && !result && (
          <section className="mt-8">
            <div className="rounded-[32px] border border-white/[0.08] bg-[#101416] p-5">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#A7F3D0]/10 text-[#A7F3D0]">
                <Link2 size={22} />
              </div>
              ...
              <p className="mt-2 text-xs leading-5 text-white/35">
                DEAD EYE will inspect the URL and explain the
                security signals it finds.
              </p>
              ...
              <button
                ...
                className="mt-4 flex w-full items-center justify-center gap-3 rounded-full bg-[#A7F3D0] px-6 py-4 font-semibold text-[#07110c] shadow-[0_10px_40px_rgba(80,230,170,0.15)] transition hover:scale-[1.01] disabled:cursor-not-allowed disabled:opacity-60"
              >
                {isAnalyzingLink ? (
                  <>
                    <Loader2
                ...
                <div className="mt-7">
                  ...
                  <div className="mt-3 space-y-2">
                    {result.analysis.evidence.length > 0 ? (
                      ...
                      <div className="rounded-2xl bg-white/[0.035] p-3 text-xs text-white/45">
        ...
        {!result && (
          <div className="mt-6 flex items-center justify-center gap-2 text-center text-[10px] text-white/25">
            ...
            {scanMode === "qr"
              ...
              : scanMode === "link"
                ? "Link analysis is processed through your local DEAD EYE security engine."
